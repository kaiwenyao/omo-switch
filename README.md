...
Chosen at repo creation time and documented here so future tooling changes are
intentional:
...
| Description | "Terminal UI for editing oh-my-openagent JSON config — vim keys, grouped model picker, atomic save." | Short, searchable. |
...
.planning/   GSD workflow artifacts (roadmap, phase plans)
```
